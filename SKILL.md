...
description: Search, brief, and monitor Korean used-market listings across 당근마켓, 번개장터, and 중고나라. Use when the user wants 중고 매물 찾아줘, 당근/번장/중고나라 동시 검색, 아이폰/맥북 같은 물건의 신규 매물 감시, 가격하락 체크, 자연어 기반 한국 중고거래 브리핑, 자연어 watch rule 추가/수정, 최근 watch 이벤트 확인, or cron-friendly stdout/JSON monitoring output.
---
...
### 3) 자연어 watch 미리보기 / 저장
```bash
python skills/used-market-watch/scripts/used_market_watch.py watch-plan "아이폰 15 프로 120만원 이하 당근 번장 신규만 감시 추가"
python skills/used-market-watch/scripts/used_market_watch.py watch-upsert "아이폰 15 프로 120만원 이하 당근 번장 신규만 감시 추가"
...
3. heartbeat/cron에서는 `watch-check --alerts-only --json`으로 신규/가격하락만 집계한다.
4. 운영 중에는 `watch-list`, `watch-events`, `watch-enable`, `watch-disable`로 상태를 관리한다.
5. 상위 레이어(OpenClaw)가 텔레그램/디스코드 전달을 담당한다.
