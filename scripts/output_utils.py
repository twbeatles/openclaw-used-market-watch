...
def render_watch_preview(payload: dict[str, Any]) -> str:
    ...
    if summary.get("event_counts"):
        counts = ", ".join(f"{k}={v}" for k, v in summary["event_counts"].items())
...
def render_watch_plan(payload: dict[str, Any]) -> str:
    ...
    lines.append(f"- 알림: {', '.join(modes) if modes else '없음'}")
...
def render_watch_list(state: dict[str, Any]) -> str:
    ...
    for rule in rules:
        ...
        lines.append(f"  · 상태: {'활성' if rule.get('enabled', True) else '비활성'} / 알림: {', '.join(modes) if modes else '없음'}")
